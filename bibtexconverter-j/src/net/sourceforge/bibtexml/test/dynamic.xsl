...
<xsl:transform version="2.0"
    ...
    exclude-result-prefixes="bibtex fn saxon xs xdt">
  ...
  <!-- @param paperbaseuri pdf links will be formed
       by appending citekeys to this uri -->
  <xsl:param name="paperbaseuri">./</xsl:param>
  <!-- @param groupby entries will be grouped by this field -->
  <xsl:param name="groupby" select="'refgroup'" />
  <xsl:param name="testint" select="1" as="xs:integer"/>
  <xsl:param name="testboolean" select="true"/>
  <xsl:template match="/">
    <html>
      <head>
...
Style sheet: <xsl:text>$Id: bibxml2htmlg.xsl,v 1.7 2006/10/26 13:33:38 Moritz.Ringler Exp $</xsl:text> by Moritz Ringler, 2003-2006
  ...
  <xsl:template match="bibtex:file">
   ...
   <li><a href="#novalue">No or empty <xsl:value-of
     select="replace($groupby, 'bibtex:', '')"/></a></li>
   </ul>
   <xsl:for-each-group select="bibtex:entry" group-by="*/*[local-name()=$groupby]">
    <xsl:sort select="current-grouping-key()" order="ascending"/>
    <xsl:if  test="current-grouping-key() ne ''">
  ...
  <h2><a name="novalue" id="novalue">No or empty <xsl:value-of
    select="replace($groupby, 'bibtex:', '')"/></a></h2>
        <table class="bibtexml-output" cellspacing="0">
       <tr>
     <th>Key</th>
     <th>Author(s)</th>
  ...
  <xsl:template match="bibtex:entry" mode="novalue">
    <xsl:if test="*/*[local-name()=$groupby] = ''">
        <xsl:call-template name="bibtex-entry">
  ...
  <xsl:template name="bibtex-entry">
    ...
    <xsl:when test="exists(bibtex:article/bibtex:pdf)">
     <a>
    <xsl:attribute name="href"><xsl:value-of select="$paperbaseuri"/>
