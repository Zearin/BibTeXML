...
"""
  ...
  4.  "author" fields with multiple authors split by " and "
      are put in separate xml "bibtex:author" tags.
  ...
  2.  Does not parse author fields into first and last names.
      ...
      In "author" tag, will show up as <bibtex:author>LAST_NAME, FIRST_NAME</bibtex:author>
  3.  Does not handle "crossref" fields other than to print <bibtex:crossref>...</bibtex:crossref>
...
def bibtexauthor(data):
    ...
    for author in author_list:
            author = author.strip()
            bibtex = bibtex + '<bibtex:author>' + removebraces(author) + \
                     '</bibtex:author>' + '\n'
    return bibtex.strip()
...
def filehandler(filepath):
    ...
    for line in washeddata
        print line
    print '  <!-- manual cleanup may be required... -->'
    outdata = bibtexdecoder(washeddata)
    print '<?xml version="1.0" encoding="iso-8859-1"?>'
    print '<!DOCTYPE bibtex:file SYSTEM "bibtexml-strict.dtd" >'
